...
std::vector<std::pair<std::string_view, size_t>> frequent_words(std::string_view data, size_t k_mer_size) {
...
std::string clump_finder(std::string_view data, size_t k_mer_length, size_t dna_range, size_t k_mer_amount) {
	std::string result{ "" };
	std::map<std::string_view, size_t> map_result{};
	size_t data_length = data.length();
	for (int i = 0; i <= data_length - dna_range; ++i) {
		std::string_view dna_substring_range = data.substr(i, dna_range);
		std::vector<std::pair<std::string_view, size_t>> k_mer_frequency = frequent_words(dna_substring_range, k_mer_length);
		for (auto& k_mer_pair_count : k_mer_frequency) {
			if (k_mer_pair_count.second >= k_mer_amount) {
				map_result[k_mer_pair_count.first];
			}
		}
	}

	for (auto& entry : map_result) {
		result += entry.first;
		result += " ";
	}
	return result;
